...
namespace YTRKDotNetCore.RestApiWithNLayer.Features.MyanmarProverbs;
...
public class MyanmarProverbsController : ControllerBase
{
    ...
    [HttpGet("{titleName}")]
    public async Task<IActionResult> Edit(string titleName)
    {
        var model = await GetDataFromApi();
    ...
    public async Task<IActionResult> GetDetail(int titleId, int proverbId)
    {
        var model = await GetDataFromApi();
